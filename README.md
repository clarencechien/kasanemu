...
Phase 1 實作,對照 `kasanemuphase1prd.md`(v1.0)。
...
跟 PRD 有出入的地方都寫在 `docs/deviations.md`,五處,每一處都有理由。
其中一處是 PRD §4.2 的襯線判定式子會把
...
```bash
npm run typecheck
npm test          # 28 個測試:選取規則、id 紀律、截斷修復、字重、快取 key
...
src/
  ...
  popup/      啟用開關、檔位、當日/當月花費
scripts/      fetch-fonts.mjs(subset 打包)  probe-gemma.mjs(模型實驗)
tests/        node:test + jsdom
docs/         fonts / acceptance / deviations
```

資料流:content script 偵測單元 → 幾何算出 `maxChars` → 可見的送進 service worker
→ 先吃快取 → 過保險絲與 token bucket → 呼叫 Gemini → id 紀律三層檢查 → 結果回 content
→ 字級分組後畫進 closed shadow DOM。捲動不重算,重排才重算。
