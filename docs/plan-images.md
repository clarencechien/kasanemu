...
- ~~veil 參數:`blur(1.4px) saturate(.55) brightness(1.16)`~~ ——
  **這一條抄錯了,已修正(§DH)**。`brightness()` 是乘法,方向寫死:
  淺底黑字上有效,深底白字上兩端都不動,原文殘留 9.7:1、譯文只有 1.2:1。
  現行配方用**方向無關**的 `contrast()`:
  `blur(2px) saturate(.35) contrast(.2) brightness(1.72)` + 淺色漸層底,
  **強度可調(0–0.6,預設 0.30)**,options 收一個滑桿。
  門檻由 `scripts/probe-veil.mjs` 在 `npm run check` 裡守著(§13-7)。
- 譯文:`Noto Sans TC` 700、**深墨橘 `#9A2A06`**(不是 chip 的 `#FF4A14`——
  veil 收出來的是亮帶,`#FF4A14` 的亮度和亮帶下緣幾乎一樣,等於沒有對比)、
  白色多層 text-shadow 光暈;四角 HUD 括號 hover 才亮。
...
| 素材 | 原文對比(裸) | 舊配方殘留 / 譯文 | 現行殘留 / 譯文 |
|---|---|---|---|
| 深底白字(bar chart) | 17.9:1 | **9.7:1 / 1.2:1** | 1.6:1 / 4.9:1 |
| 淺底黑字(白底圖表) | 18.9:1 | 7.2:1 / 6.9:1 | 1.4:1 / 7.6:1 |
| 中灰底深字(照片) | 3.9:1 | 2.9:1 / 2.7:1 | 1.2:1 / 6.1:1 |
| 彩底白字(品牌色塊) | 6.7:1 | 4.2:1 / **1.1:1** | 1.4:1 / 5.5:1 |

舊配方那兩格粗體就是使用者回報的「左上那個字根本不能看」——
不能看的是**加註本身**(1.2:1),而原文照樣讀得到(9.7:1)。
