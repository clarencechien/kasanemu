...
## EA. 放大檢視的入口有時在有時不在,預算跟著畫布走

> 首先外面的翻譯 只有翻一部分 這是我們設定只有 5%的預算?
> 點tip 開視窗後 又多出了好幾個翻譯 這個預算是5%? 因為圖放大了?
> 這個一開始送出去回來就有10 20個翻譯嗎 只是看預算有多少才放上去嗎?
> 那在視窗裡是一個預算 外面是一個預算嗎?

使用者對著 claude.com 的產品截圖問了四個問題,每一個都值得寫下答案:

1. **行內只翻一部分是預算,不是 bug。** 模型一次回**全部的塊**
   (那張圖 15+ 塊),快取住;行內畫幾塊是顯示端的事 ——
   塊數上限(§DX)+ 面積 18% 保險絲 + 不重疊,三道閘門。
2. **點開放大檢視多出來的譯文,就是同一份快取換大畫布再排一次**
   `placeBlocks` —— 零成本、零請求。「因為圖放大了」猜得完全正確。
3. **是,回來就全有了,預算只決定放上去幾塊。**
4. **以前裡外是同一條規則**:面積預算隨畫布放大自動變鬆(這是面積的
   物理),但塊數上限不會 —— 裡外都卡同一個數字,放大了也多不了幾塊。

### 盤點:有站方 lightbox 的站 vs 沒有的站

| | 有站方 lightbox(ClickHouse 那型) | 沒有(thenewstack 文章圖) |
|---|---|---|
| 入口 | 站方自己的(§2.4,不出第二顆) | 以前:有塊沒放下才出 → **現在:翻了就出** |
| 大圖上的加註 | 同 src 認親跟過去 | 我們的黑底放大檢視 |
| 大畫布預算 | 畫布 ≥900px 自動 ×2(placeBlocks 的尺寸閘門,不用另外接線) | 同左 |

**決定(使用者):翻譯後統一有黑底視窗;圖大預算多、圖小預算少;
上限做成設定。**

- `settings.imageMaxPlates`(預設 6,設定頁「一張圖最多疊幾塊」):
  行內的塊數上限。這是品味閘門,面積與不重疊仍是保險絲。
- `placeBlocks` 收畫布寬:**≥ `BIG_CANVAS_W`(900px)就放兩倍** ——
  「圖放大了預算就多」收成一個尺寸閘門,行內小圖、放大檢視、
  站方 lightbox 的大圖全走同一條規則。
- 放大檢視的入口從「有塊沒放下才出」改成「**畫了東西就出**」
  (站方有 lightbox 仍然讓路)。入口時有時無看起來像亂數 ——
  統一之後「點 tip 開視窗」變成一個可以依賴的動作。

### 順手修:Alt 按了沒反應(Windows 的右 Alt)

> 這時 按了alt也不會消失?

使用者這幾天從 ChromeOS 換到 Windows。Windows 上**右 Alt 送來的
`key` 是 `'AltGraph'`**,不是 `'Alt'` —— 收層的判斷只認左邊那顆。
